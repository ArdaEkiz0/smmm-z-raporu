"""
E-Fatura mükellef sorgu modülü.

GİB e-fatura/e-arşiv mükellef sorgusu yapar.
NOT: GİB e-fatura mükellef sorgu API'si artık doğrudan erişilebilir
değildir (CAPTCHA + e-Devlet doğrulaması gerektirir). Sorgu için bir
e-Fatura entegratörü (Foriba, Mikro, vs.) kullanılması önerilir.

Kaynak: https://www.efatura.gov.tr
"""
import re
import time
...
def gib_efatura_sorgula(vkn: str, timeout: float = 10.0) -> Dict:
    ...
    NOT: GİB e-fatura mükellef sorgu API'si artık public erişime kapalıdır
    (CAPTCHA + e-Devlet doğrulaması gerektirir). Bu fonksiyon yine de
    GİB public endpoint'ini dener; başarısız olursa anlamlı hata döner.
    ...
    sonuc["hata"] = sonuc["hata"] or (
        "GİB e-fatura mükellef sorgu API'si doğrudan erişilebilir değil. "
        "Sorgu için bir e-Fatura entegratörü (Foriba, Mikro, vs.) kullanılması önerilir."
    )
    return sonuc
